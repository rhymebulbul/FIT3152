...
The previous code is reused, but with the `rem` dataset instead of `croatia`, to repeat the same task for the rest of the world. Initially, this dataset's correlation matrix is displayed. In order to keep this report as short as possible, variations of code that has been reused will now appear in the **Appendix**.
